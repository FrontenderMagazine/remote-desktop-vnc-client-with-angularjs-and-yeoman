...
Код, используемый в данной статье доступен на [github][4]. [Здесь][5]
можно увидеть законченный пример.

Это уже мой третий проект, опубликованный на [github][6], который посвящен
протоколам удаленного доступа. ([VNC клиент за 200 строчек на JavaScript][7], [VNC клиент
...
поддерживать [RFB протокол (Remote FrameBuffer)][10].  В центре схемы — 
прокси-сервер, с установленным RFB-клиентом, который умеет общаться с RFB-сервером.
Прокси-сервер также работает как HTTP-сервер, раздающий статические
файлы, а также умеет общаться при помощи [socket.io][11]. Последний
...
прокси-сервере. Это — то, что видит пользователь VNC клиента. Пользователи используют
...
скопировать код прокси с [github][13].

Начнем с прокси-сервера. Создадим директорий `angular-vnc`. Внутри —
директорий `proxy`.
    ...
    mkdir proxy
    cd proxy
    # Создадим package.json, инициализируя nodejs-приложение
    npm init
...
В приведенном выше фрагменте кода создается новое приложение используя фреймворк Express,
слушающее на порту `8090` и раздающее статику из папки `/../../client/app`.

Следующим шагом, обернем HTTP сервер в socket.io, добавим обработчиков событий
для входящих соединений и инициализируем переменную `clients`. В
...
получаем кадр — значение переменной равно false (это значение переменных 
булева типа по умолчанию — подробности можно узнать в статье «[отложенные вычисления][14]»),
...
Они задают разметку и стили для простенькой формы на Bootstrap-е.
...
Если вы все еще не поняли как работает валидация, ознакомьтесь со статьей о
[валидации форм][17].
...
вызываем метод `form.$setDirty();`. Мы делаем это для того, чтобы удалить 
класс `ng-pristine` формы и провести валидацию повторно. Это необходимо в случае, если
пользователь не введет ничего и нажмет кнопку `Login`. Если форма валидна, мы
вызываем подключение к VNC клиенту. Как вы видите, данная функция возвращает
промис, при разрешении которого мы редиректим на страницу `http://localhost:8090/#/vnc`, 
а при ошибке мы показываем пользователю сообщение `Connection timeout. Please, try again.` 
(посмотрите в html-разметке `<div class="form-error" ng-bind="errorMessage"></div>`).

Следующий компонент, который мы собираемся рассмотреть - это сервис
VNCClient. Давайте создадим его при помощи `Yeoman`
...
могли заметить, что мы не следуем лучшему способу определения конструктора
...
от прокси. При получении ответа за время меньшее таймаута, мы выполняем коллбек
для успешного выполнения промисса, созданный в начале метода `connect`.

Таким способом мы трансформируем интерфейс на основе коллбеков,
предоставляемый библиотекой socket.io в интерфейс, использующий промисы.
...
Давайте создадим [директиву][20] VNC screen:

    yo angular:directive vnc-screen

И откроем файл `app/scripts/directives/vnc-screen.js`. 
Наша директива выглядит следующим образом:
...
VNCClient.screenHeight)` создаст скрытый canvas. Его размеры соответствуют
размерам удаленного экрана. Так, если экран удаленного монитора 1024x768px
...
screen` (оболочки для скрытого канваса). Делать мы это будем потому, что можем
изменить размер так, что бы он отличался от разрешения экрана на удаленной машине 
и мы будем отрисовывать каждый новый фрейм с учетом коэффициента масштабирования.
...
клавиши мы вызываем коллбеки с null контекстом. Чтобы обойти это, мы
принудительно привязываем контекст к `Vncclient`.

Все, закончили! Мы пропустили несколько методов объекта `Screen`, так как их
рассмотрение неважно для этой статьи. В любом случае здесь приведена полная 
имплементация `Screen`:
